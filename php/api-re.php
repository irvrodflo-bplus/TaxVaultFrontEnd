...
class CFDIApi {
    ...
    private function testearConexion() {
        try {
            ...
            // Test de tabla cfdi_recibidos
            $sql = "SHOW TABLES LIKE 'cfdi_recibidos'";
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute();
            $table_exists = $stmt->fetch();
            
            if (!$table_exists) {
                throw new Exception("La tabla 'cfdi_recibidos' no existe");
            }
            
            // Contar registros
            $sql = "SELECT COUNT(*) as total FROM cfdi_recibidos";
            $stmt = $this->pdo->prepare($sql);
            $stmt->execute();
    ...
    private function obtenerCFDIs() {
        try {
            ...
            $sql = "SELECT 
                        ...
                        cer.codigo_estatus,
                        cer.es_cancelable,
                        cer.estatus_cancelacion,
                        cer.validacion_efos,
                        cer.fecha_consulta,
                        cer.fecha_actualizacion
                    FROM cfdi_recibidos cr
                    LEFT JOIN cfdi_estados_r cer ON cer.id_cfdi = cr.uuid
                    ORDER BY cr.fecha_expedicion DESC, cr.created_at DESC
            ...
            $sqlCount = "SELECT COUNT(*) as total FROM cfdi_recibidos";
    ...
    private function filtrarCFDIs() {
        try {
            ...
            $sqlCount = "SELECT COUNT(*) as total FROM cfdi_recibidos {$whereClause}";
            $stmtCount = $this->pdo->prepare($sqlCount);

...
const DataManager = {
    ...
    actualizarTabla: function () {
        ...
        if (filteredData.length === 0) {
            tbody.innerHTML = '<tr><td colspan="18" class="text-center">No se encontraron registros</td></tr>';
        ...
        pageData.forEach((cfdi, index) => {
            ...
            row.innerHTML = `
                ...
                <td title="${cfdi.nombre_receptor}">${Utils.truncarTexto(cfdi.nombre_receptor, 25)}</td>
                <td>${cfdi.serie || '-'}</td>
                <td>${cfdi.folio || '-'}</td>
...
function exportarExcel() {
    ...
    try {
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'CFDIs Emitidos');
